...
function normalizeUrl(url) {
  ...
  if (!s) return '';
  // Users often paste edu-lokalmart.odoo.com without protocol.
  // The xmlrpc package can silently treat protocol-less URLs as localhost,
  // which causes ECONNREFUSED 127.0.0.1:80. Always force an explicit protocol.
  if (!/^https?:\/\//i.test(s)) s = 'https://' + s.replace(/^\/+/g, '');
  s = s.replace(/\/+$/, '');
  try {
    const u = new URL(s);
...
function validateOdooUrl(url) {
  if (!url) throw new Error('URL Odoo wajib diisi, contoh: https://edu-lokalmart.odoo.com');
  let parsed;
  try { parsed = new URL(url); }
  catch { throw new Error(`URL Odoo tidak valid: ${url}. Gunakan format https://domain-odoo.odoo.com`); }
  if (!['http:', 'https:'].includes(parsed.protocol)) {
    throw new Error(`URL Odoo harus memakai http:// atau https://, bukan ${parsed.protocol}`);
  }
  const blocked = new Set(['localhost', '127.0.0.1', '0.0.0.0', '::1']);
  if (blocked.has(parsed.hostname)) {
    throw new Error('URL Odoo mengarah ke localhost/127.0.0.1. Isi URL server Odoo yang benar, misalnya https://edu-lokalmart.odoo.com');
  }
  return parsed;
}
