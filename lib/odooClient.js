...
const { normalizeUrl, chunkArray } = require('./utils');
...
class OdooClient {
  constructor(config) {
    ...
    this.username = config.username || config.user || process.env.ODOO_USERNAME;
    this.password = config.password || config.apiKey || process.env.ODOO_PASSWORD;
    if (!this.url || !this.db || !this.username || !this.password) {
      throw new Error('Konfigurasi Odoo belum lengkap: url, db, username, password/apiKey wajib diisi.');
    }
