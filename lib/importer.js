...
function ensureImportCache(options = {}) {
  if (!options._cache) options._cache = {};
  const c = options._cache;
  if (!c.fieldsMeta) c.fieldsMeta = new Map();
  if (!c.externalId) c.externalId = new Map();
  if (!c.displayName) c.displayName = new Map();
  if (!c.modelId) c.modelId = new Map();
  return c;
}

function fastContext(options = {}) {
  const base = {
  ...
  return { context: { ...base, ...(options.context || {}) } };
}

async function modelFields(client, model, options = {}) {
  const cache = ensureImportCache(options);
  if (cache.fieldsMeta.has(model)) return cache.fieldsMeta.get(model);
  const meta = await client.fieldsGet(model, [], ['type', 'relation', 'required', 'readonly', 'selection', 'string']).catch(() => ({}));
  cache.fieldsMeta.set(model, meta);
  return meta;
}

async function resolveExternalIdCached(client, xmlid, expectedModel = null, options = {}) {
  const value = String(xmlid || '').trim();
  if (!value) return false;
  const cache = ensureImportCache(options);
  const key = `${expectedModel || '*'}|${value}`;
  if (cache.externalId.has(key)) return cache.externalId.get(key);
  const id = await resolveExternalId(client, value, expectedModel);
  cache.externalId.set(key, id || false);
  return id || false;
}

async function findByExternalIdCached(client, xmlid, options = {}) {
  const value = String(xmlid || '').trim();
  if (!value) return null;
  const cache = ensureImportCache(options);
  const key = `find|${value}`;
  if (cache.externalId.has(key)) return cache.externalId.get(key);
  const found = await findByExternalId(client, value);
  cache.externalId.set(key, found || null);
  return found || null;
}
...
async function ensureManualModel(client, row) {
  ...
  if (existing.length) {
    await client.write('ir.model', [existing[0].id], vals, fastContext({}));
  ...
  const id = await client.create('ir.model', vals, fastContext({}));
...
async function ensureManualField(client, row) {
  ...
  const cache = ensureImportCache(options);
  let modelId = cache.modelId.get(model);
  if (!modelId) {
    ...
    cache.modelId.set(model, modelId);
  }
  ...
  if (existing.length) {
    ...
    if (options.skipExistingCustomFields !== false) {
      id = existing[0].id;
      result = { skipped: true, reason: 'Field custom sudah ada, update dilewati untuk mempercepat import.', id, model, name };
    } else {
      await client.write('ir.model.fields', [existing[0].id], vals, fastContext(options));
      id = existing[0].id;
      result = { updated: true, id, model, name };
    }
  } else {
    id = await client.create('ir.model.fields', vals, fastContext(options));
...
async function createOrUpdateXmlId(client, xmlid, model, resId) {
  ...
  if (existing.length) {
    await client.write('ir.model.data', [existing[0].id], { model, res_id: resId, noupdate: true }, fastContext({}));
    return existing[0].id;
  }
  return client.create('ir.model.data', { module: parts.module, name: parts.name, model, res_id: resId, noupdate: true }, fastContext({}));
}
...
async function resolveFieldId(client, row) {
  ...
  if (fieldXmlId) {
    const id = await resolveExternalIdCached(client, fieldXmlId, 'ir.model.fields', row._options || {});
    if (id) return id;
  }
...
async function ensureFieldSelection(client, row) {
  ...
  let id;
  if (existing.length) {
    await client.write('ir.model.fields.selection', [existing[0].id], vals, fastContext({}));
    id = existing[0].id;
  } else {
    id = await client.create('ir.model.fields.selection', vals, fastContext({}));
  }
...
async function resolveByDisplayName(client, model, name, options = {}) {
  const value = String(name || '').trim();
  if (!value) return false;
  const cache = ensureImportCache(options);
  const key = `${model}|${value}`;
  if (cache.displayName.has(key)) return cache.displayName.get(key);
  ...
  for (const domain of domainCandidates) {
    try {
      ...
      if (rows.length) { cache.displayName.set(key, rows[0].id); return rows[0].id; }
    } catch (_) {}
  }
  cache.displayName.set(key, false);
  return false;
}

async function convertValue(client, fieldsMeta, field, raw, row, options = {}) {
  ...
  if (type === 'many2one') {
    ...
    let resolved = await resolveExternalIdCached(client, rawString, meta.relation, options);
    if (!resolved && !rawString.includes('.')) resolved = await resolveByDisplayName(client, meta.relation, rawString, options);
  ...
  if (type === 'many2many') {
    ...
    for (const ref of values) {
      let id = await resolveExternalIdCached(client, ref, meta.relation, options);
      if (!id && !ref.includes('.')) id = await resolveByDisplayName(client, meta.relation, ref, options);
      if (id) ids.push(id);
...
async function rowToVals(client, model, fieldsMeta, row, options = {}) {
  const vals = {};
  const warnings = [];
  ...
  for (const [col, raw] of Object.entries(normalized)) {
    ...
    if (col.endsWith('_name_hint')) {
      ...
      if (fieldsMeta[actualField] && fieldsMeta[actualField].type === 'many2one' && raw) {
        const converted = await convertValue(client, fieldsMeta, actualField, raw, normalized, options);
    ...
    if (col.endsWith('_external_id')) {
      ...
      const converted = await convertValue(client, fieldsMeta, field, raw, normalized, options);
    ...
    const converted = await convertValue(client, fieldsMeta, col, raw, normalized, options);
    if (converted.error) throw new Error(converted.error);
...
async function upsertRow(client, model, row, options = {}) {
  ...
  if (model === 'ir.model.fields') return ensureManualField(client, { ...normalized, _options: options });
  if (model === 'ir.model.fields.selection') return ensureFieldSelection(client, { ...normalized, _options: options });

  const fieldsMeta = await modelFields(client, model, options);
  const { vals, warnings } = await rowToVals(client, model, fieldsMeta, normalized, options);
  const action = String(normalized.__action || 'upsert').toLowerCase();
  const xmlid = String(normalized._external_id || '').trim();
  let target = xmlid ? await findByExternalIdCached(client, xmlid, options) : null;
  ...
  if (target) {
    if (action === 'create' || options.skipExistingRecords) return { skipped: true, reason: options.skipExistingRecords ? 'Record sudah ada, update dilewati untuk mempercepat import.' : '__action=create tetapi External ID sudah ada.', id: target.res_id, warnings };
    await client.write(model, [target.res_id], vals, fastContext(options));
    return { updated: true, id: target.res_id, warnings };
  }
  if (action === 'update') return { skipped: true, reason: '__action=update tetapi External ID belum ada.', warnings };
  const id = await client.create(model, vals, fastContext(options));
...
async function importRowsJson(client, model, rowsInput = [], options = {}) {
  ...
  for (let i = 0; i < limitedRows.length; i++) {
    ...
    try {
      const result = await upsertRow(client, model, row, runOptions);
      item.processed++;
