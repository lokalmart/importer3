...
function syntheticXmlId(model, id, moduleName = 'lokalmart_mig') {
  const safeModel = String(model || 'record').replace(/[^a-zA-Z0-9_]/g, '_').replace(/_+/g, '_');
  return `${moduleName}.${safeModel}_${Number(id) || id}`;
}
...
async function relationMapsForRows(client, model, rows, fieldsMeta, exportFields, options = {}) {
  ...
  for (const [relModel, idsSet] of Object.entries(relationIdsByModel)) {
    ...
    if (options.syntheticExternalIds !== false) {
      for (const id of idsSet) {
        if (!maps[relModel].has(id)) maps[relModel].set(id, syntheticXmlId(relModel, id, options.module || 'lokalmart_mig'));
      }
    }
  }
  return maps;
...
async function exportModelRows(client, model, options = {}) {
  ...
  if (options.ensureExternalIds) {
    for (const r of rawRows) {
      if (!xmlidMap.has(r.id)) {
        const xmlid = await ensureExternalId(client, model, r.id, moduleName);
  ...
  if (options.syntheticExternalIds !== false) {
    for (const r of rawRows) {
      if (!xmlidMap.has(r.id)) xmlidMap.set(r.id, syntheticXmlId(model, r.id, moduleName));
    }
  }
  const relationMaps = await relationMapsForRows(client, model, rawRows, fieldsMeta, exportFields, { ...options, module: moduleName });
...
function relationMapFromExportRows(model, rows = []) {
  const cols = new Set();
  for (const row of rows.slice(0, 50)) {
    for (const col of Object.keys(row || {})) {
      if (col.endsWith('_external_id') || col.endsWith('_external_ids')) cols.add(col);
    }
  }
  return Array.from(cols).sort().map(col => {
    const many = col.endsWith('_external_ids');
    return {
      model,
      field: col.replace(/_external_ids?$/, ''),
      export_column: col,
      relation_type: many ? 'many2many' : 'many2one',
      relation_model: 'resolved_by_external_id'
    };
  });
}
...
async function exportModelRowsJson(client, model, options = {}) {
  const limit = Math.max(1, Math.min(Number(options.limit || options.batchSize || 50), Number(options.maxLimit || 200)));
  ...
  const pageOptions = {
    ...
    ensureExternalIds: options.ensureExternalIds === true,
    syntheticExternalIds: options.syntheticExternalIds !== false,
    module: options.module || 'lokalmart_mig',
    readChunkSize: Math.max(5, Math.min(Number(options.readChunkSize || 20), 50))
  };
...
async function exportSingleModelXlsx(client, model, options = {}) {
  const limit = Math.max(1, Math.min(Number(options.limit || options.batchSize || 50), Number(options.maxLimit || 500)));
...
async function exportMigrationXlsx(client, options = {}) {
  ...
  for (const model of models) {
    ...
    try {
      ...
      relationMap.push(...relationMapFromExportRows(model, result.rows));
    } catch (err) {
  ...
  addJsonSheet(wb, '00_relation_map', relationMap.length ? relationMap : [{ model: '', field: '', export_column: '', relation_type: '', relation_model: '' }]);
