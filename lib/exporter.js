...
const { getProfile, exportModelsForProfile } = require('./modelProfiles');
...
const { SYSTEM_FIELDS, uniq, nowIso, flattenError } = require('./utils');
...
function pickExportFields(model, fieldsMeta, options = {}) {
  ...
  if (profile.includeFields) return profile.includeFields.filter(f => fieldsMeta[f]);
  const out = [];
...
async function exportModelRows(client, model, options = {}) {
  ...
  const domain = options.domain || profile.domain || [];
  const limit = options.limit || profile.limit || 1000;
  const ids = await client.search(model, domain, { limit, order: options.order || 'id asc' });
...
async function exportMigrationXlsx(client, options = {}) {
  const models = options.models || exportModelsForProfile(options.profile || 'full');
  ...
  return {
    filename: `lokalmart_migration_${options.profile || 'full'}_${new Date().toISOString().slice(0, 10)}.xlsx`,
...
async function exportMigrationPackageZip(client, options = {}) {
  ...
  return {
    filename: `lokalmart_migration_package_${options.profile || 'full'}_${new Date().toISOString().slice(0, 10)}.zip`,
